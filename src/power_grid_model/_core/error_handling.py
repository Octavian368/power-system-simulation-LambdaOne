...
_INVALID_BRANCH_RE = re.compile(
    r"Branch (-?\d+) has the same from- and to-node (-?\d+),\n This is not allowed!\n"
)
...
_INVALID_TRANSFORMER_CLOCK_RE = re.compile(
    r"Invalid clock for transformer (-?\d+), clock (-?\d+)\n"
)
...
_ITERATION_DIVERGE_RE = re.compile(
    r"Iteration failed to converge"
...
_INVALID_MEASURED_OBJECT_RE = re.compile(
    r"(\w+) measurement is not supported for object of type (\w+)"
)
...
_INVALID_CALCULATION_METHOD_RE = re.compile(
    r"The calculation method is invalid for this calculation!"
)
_INVALID_SHORT_CIRCUIT_PHASE_OR_TYPE_RE = re.compile(
    r"short circuit type"
)  # multiple different flavors
_POWER_GRID_DATASET_ERROR_RE = re.compile(r"Dataset error: ")  # multiple different flavors
_POWER_GRID_UNREACHABLE_HIT_RE = re.compile(
    r"Unreachable code hit when executing "
)  # multiple different flavors
_POWER_GRID_SEARCH_OPT_INCMPT_RE = re.compile(
    r"Search method is incompatible with optimization strategy: "
)
_POWER_GRID_NOT_IMPLEMENTED_ERROR_RE = re.compile(
    r"The functionality is either not supported or not yet implemented!"
)
...
def find_error(batch_size: int = 1, decode_error: bool = True) -> RuntimeError | None:
    ...
    if error_code == PGM_BATCH_ERROR:
        ...
        error.errors = [
            _interpret_error(message, decode_error=decode_error) for message in error.error_messages
        ]

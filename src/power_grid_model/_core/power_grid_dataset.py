...
class CMutableDataset:
    ...
    def __new__(cls, data: Dataset, dataset_type: Any = None):
        ...
        instance._dataset_type = (
            dataset_type if dataset_type in DatasetType else get_dataset_type(data)
        )
        ...
        if data:
            ...
            first_sub_info = get_buffer_properties(
                data=first_component_data, schema=instance._schema[first_component]
            )
    ...
    def _add_component_data(
        self, component: ComponentType, data: ComponentData, allow_unknown: bool = False
    ):
        """
    ...
    def _validate_properties(self, data: ComponentData, schema: ComponentMetaData):
        properties = get_buffer_properties(
            data, schema=schema, is_batch=self._is_batch, batch_size=self._batch_size
        )
        ...
        if properties.batch_size != self._batch_size:
            raise ValueError(
                f"Dataset must have a consistent batch size across all components. {VALIDATOR_MSG}"
            )
...
class CConstDataset:
    ...
    def __new__(cls, data: Dataset, dataset_type: DatasetType | None = None):
        ...
        instance._const_dataset = pgc.create_dataset_const_from_mutable(
            mutable_dataset.get_dataset_ptr()
        )
        assert_no_error()
...
class CWritableDataset:
    ...
    def _get_buffer_properties(
        self, info: CDatasetInfo
    ) -> Mapping[ComponentType, BufferProperties]:
    ...
    def _filter_with_mapping(self):
        for component_type, component_buffer in self._data.items():
            if component_type in self._data_filter:
                ...
                if filter_option is ComponentAttributeFilterOptions.relevant and is_columnar(
                    component_buffer
                ):
...
def _get_filtered_attributes(
    ...
    if isinstance(component_data_filter, ComponentAttributeFilterOptions):
        if (
            component_data_filter == ComponentAttributeFilterOptions.relevant
            and attribute_indication is not None
        ):

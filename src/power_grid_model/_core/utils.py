...
def convert_batch_dataset_to_batch_list(
    batch_data: BatchDataset, dataset_type: DatasetType | None = None
) -> BatchList:
    """
    ...
    for component, data in batch_data.items():
        ...
        if is_sparse(data):
            component_batches = split_sparse_batch_data_in_batches(
                cast(SparseBatchData, data), component
            )
        else:
            component_batches = split_dense_batch_data_in_batches(
                cast(SingleComponentData, data), batch_size=n_batches
            )
        for i, batch in enumerate(component_batches):
            if (isinstance(batch, dict) and batch) or (
                isinstance(batch, np.ndarray) and batch.size > 0
            ):
                list_data[i][component] = batch
    return list_data


def get_and_verify_batch_sizes(
    batch_data: BatchDataset, dataset_type: DatasetType | None = None
) -> int:
    """
    ...
    if dataset_type is None and any(
        is_columnar(v) and not is_sparse(v) for v in batch_data.values()
    ):
...
def get_batch_size(
    ...
    if not is_columnar(batch_data):
        ...
        if component is None or dataset_type is None:
            raise ValueError(
                "Cannot deduce batch size for given columnar data without a dataset type or component"
            )
...
def split_dense_batch_data_in_batches(
    ...
    scenarios_per_attribute = {
        attribute: _split_numpy_array_in_batches(attribute_data)
        for attribute, attribute_data in data.items()
    }

    return [
        {
            attribute: scenarios_per_attribute[attribute][scenario]
            for attribute, attribute_data in data.items()
        }
    ...
    for comp_name, attrs in processed_data_filter.items():
        ...
        if is_sparse(data[comp_name]):
            result_data[comp_name] = {
                "indptr": _extract_indptr(data[comp_name]),
                "data": converted_sub_data,
            }
        else:
...
def _convert_data_to_row_or_columnar(
    ...
    if isinstance(attrs, ComponentAttributeFilterOptions):
        names = (
            cast(SingleArray, data).dtype.names
            if not is_columnar(data)
            else cast(SingleColumnarData, data).keys()
        )
...
def _check_sparse_dense(component_data: ComponentData, err_msg_suffixed: str) -> ComponentData:
    if is_sparse(component_data):
        ...
        if not isinstance(indptr, np.ndarray):
            raise TypeError(
                err_msg_suffixed.format(f"Invalid indptr type {type(indptr).__name__}. ")
            )
        sub_data = component_data["data"]
    elif isinstance(component_data, dict) and (
        "indptr" in component_data or "data" in component_data
    ):
        missing_element = "indptr" if "indptr" not in component_data else "data"
        raise KeyError(
            err_msg_suffixed.format(f"Missing '{missing_element}' in sparse batch data. ")
        )
    else:
    ...
    if is_columnar(sub_data):
        ...
        for attribute, attribute_array in sub_data.items():
            ...
            if attribute_array.ndim not in [1, 2, 3]:
                raise TypeError(
                    err_msg_suffixed.format(f"Invalid dimension: {attribute_array.ndim}")
                )
...
def _extract_data_from_component_data(data: ComponentData, is_batch: bool | None = None):
    return (
        _extract_columnar_data(data, is_batch)
        if is_columnar(data)
        else _extract_row_based_data(data, is_batch)
    )
...
def get_dataset_type(data: Dataset) -> DatasetType:
    ...
    if all(is_columnar(v) for v in data.values()):
        raise ValueError(
            "The dataset type could not be deduced. At least one component should have row based data."
        )
    ...
    for dataset_type, dataset_metadatas in power_grid_meta_data.items():
        for component, dataset_metadata in dataset_metadatas.items():
            ...
            component_dtype = (
                component_data["data"].dtype if is_sparse(component_data) else component_data.dtype
            )
    ...
    if len(candidates) > 1:
        raise ValueError(
            "The dataset type could not be deduced because multiple dataset types match the data."
        )

...
class Serializer(ABC):
    ...
    def __new__(
        ...
        instance._serializer = pgc.create_serializer(
    ...
    def dump_str(self, *, use_compact_list: bool = False, indent: int = 2) -> str:
        ...
        """
        data = pgc.serializer_get_to_zero_terminated_string(
            self._serializer, int(use_compact_list), indent
        )
        assert_no_error()
        return data
    ...
    def dump_bytes(self, *, use_compact_list: bool = False) -> bytes:
        ...
        pgc.serializer_get_to_binary_buffer(
            self._serializer, int(use_compact_list), byref(raw_data), byref(size)
        )
        assert_no_error()
...
def json_serialize(
    ...
    result = JsonSerializer(data=data, dataset_type=dataset_type).dump(
        use_compact_list=use_compact_list, indent=indent
    )
    assert_no_error()
    return result
...
def msgpack_serialize(
    ...
    if dataset_type is not None:
        dataset_type = _str_to_datatype(dataset_type)
    result = MsgpackSerializer(data=data, dataset_type=dataset_type).dump(

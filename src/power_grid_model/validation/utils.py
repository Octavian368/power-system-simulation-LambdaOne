...
def _eval_field_expression(data: np.ndarray, expression: str) -> np.ndarray:
    ...
    assert len(fields) == 2
    zero_div = np.logical_or(
        np.equal(data[fields[1]], 0.0), np.logical_not(np.isfinite(data[fields[1]]))
    )
    if np.any(zero_div):
...
def _update_component_data(
    ...
    if isinstance(input_data, np.ndarray) and isinstance(update_data, np.ndarray):
        return _update_component_array_data(
            component=component, input_data=input_data, update_data=update_data
        )
...
def errors_to_string(
    ...
    if isinstance(errors, dict):
        return "\n".join(
            errors_to_string(err, f"{name}, batch #{i}", details)
...
def _nan_type(
    component: _ComponentTypeLike, field: str, data_type: DatasetType = DatasetType.input
):
    """
...
def _get_indexer(
    source: np.ndarray, target: np.ndarray, default_value: int | None = None
) -> np.ndarray:
    """
    ...
    indices = np.searchsorted(
        source, target, sorter=permutation_sort
    ...
    return np.where(
        source[clipped_indices] == target, permutation_sort[clipped_indices], default_value
    )


def _set_default_value(
    data: SingleDataset,
    component: _ComponentTypeLike,
    field: str,
    default_value: int | float | np.ndarray,
):
    """
...
def _get_valid_ids(
    data: SingleDataset, ref_components: _ComponentTypeLike | list[ComponentTypeVar]
) -> list[int]:
    """
...
def _get_mask(
    data: SingleDataset, component: _ComponentTypeLike, field: str, **filters: Any
) -> np.ndarray:
    """
